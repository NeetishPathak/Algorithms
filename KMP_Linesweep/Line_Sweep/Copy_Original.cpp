...
public:
	...
	Point p2;
	int lineId;
	float yCoord; //variable used for comparing linesegments in the set
...
Point intersectionPoint(LineSegment l1, LineSegment l2){
	...
	//cout << x << " " << y << endl;
	//cout << "-------------------" << endl;
	//intersection point was found from two linesegments l1 and l2
	Point retP = Point(x,y,-1,-1);
	retP.firstLine = l1.lineId;
	retP.secLine = l2.lineId;
//	cout << "Line id 1 returned is " << retP.firstLine << endl;
//	cout << "Line id 2 returned is " << retP.secLine << endl;
	return retP;
 ...
 * Returns: void
 * Description: sweepLineIntersection algo
 * */
...
//			lineList[firstLineSegmentId].yCoord = lineList[firstLineSegmentId].setYCoord(p.x + 0.1);
//			lineList[secondLineSegmentId].yCoord = lineList[secondLineSegmentId].setYCoord(p.x + 0.1);

//			cout << "After swapping the y coords" << endl;
...
/*Function: Naive way to find intersection
 * */
void naiveWay(vector<LineSegment> lList){
...
int main(){
	...
	//Show the prepared pointList
	//showPointList(pointsList);

	//sort the pointsList based on value of x axis
	sortPointListX(pointsList);

	//assign priority to each line based on left y coordinate
	//sortLineListY(lineList);

	//Show the pointList after sorting
	//showPointList(pointsList);

	//testLine intersection
	//tesLineIntersectionFunction();

	//Show the list of all the lines
	//showLineList(lineSegmentList);

	//sort the line segments as per y axis
...
//working code
	/*try naive way
	naiveWay(lineSegmentList); //This is working fine
	*/

//Trial

//		for(int i=0; i < lineSegmentLevelProritized.size(); ++i){
//			lineSegmentLevelProritized[i].printLineSegment();
//		}
