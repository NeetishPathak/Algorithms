...
vector<Point> sweepLineIntersectionAllPoints2(vector<Point> pList, vector<LineSegment> lineList){
	...
	bool(*cmp_XAgain)(Point,Point) = myCompareSmall;
...
//								}
			...
			std::set<LineSegment,bool(*)(LineSegment,LineSegment)>::iterator higherItr = SL.end();
			std::set<LineSegment,bool(*)(LineSegment,LineSegment)>::iterator lowerItr = SL.end();
			...
			lineList[firstLineSegmentId].yCoord = yCoordSecond;//lineList[firstLineSegmentId].setYCoord(yCoordSecond);
			lineList[secondLineSegmentId].yCoord = yCoordFirst;//lineList[secondLineSegmentId].setYCoord(yCoordFirst);
			...
			if(firstLS.yCoord > secondLS.yCoord){
				...
				showSL(SL);
				cout << "Removing firstLS from SL" << endl;
				firstLS.printLineSegment();
				SL.erase(firstLS);
				showSL(SL);
				//secondLine has the lower yCoord
				cout << "Removing secondLS from SL" << endl;
				firstLS.printLineSegment();
				SL.erase(secondLS);
				showSL(SL);
				cout << "Inserting a new lineList[secondLineSegmentId] in higer position" << endl;
				...
				showSL(SL);
				cout << "Inserting a new lineList[firstLineSegmentId] in lower poistion" << endl;
			...
			cout << "swapping is done now do more comparisons" << endl;
			...
			if(segHH != SL.end()){
				...
				if(doLinesIntersect(lineList[segHH->lineId],lineList[lowerItr->lineId])){
					...
					cout << "segHH and lowerItr intersect" << endl;

//					if(PQ.find(intPoint) != PQ.end()){
						cout << "Erase from PQ the point" << endl;
						intPoint.printPoint();
						PQ.erase(intPoint);
//					}
				}
...
//					if(PQ.find(intPoint) != PQ.end()){
					cout << "segLL and higherItr intersect" << endl;
					cout << "Erase from PQ the point" << endl;
					intPoint.printPoint();
						PQ.erase(intPoint);
//					}
...
//				cout << "This is hit4" << endl;
				if(doLinesIntersect(lineList[segLL->lineId],lineList[lowerItr->lineId])){
					cout << "segLL and lowerItr intersect at" << endl;
...
//					}
		...
		cout << "Point that is now fully processed is " << endl;
		p.printPoint();
		PQ.erase(p);
		cout << "Check for urself" << endl;
		showPQ(PQ);
		showSL(SL);
	}
